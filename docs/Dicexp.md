...
- `or`（两个值之间的或运算），如：`false or true` 结果为 `true`。
- `and`（两个值之间的与运算），如：`false and true` 结果为 `false`。
- `not`（单个值自身的非运算），如：`not false` 结果为 `true`。
...
Dicexp 提供了许多内建的通常函数。对于通常函数，一般习惯以
“`<函数名>/<参数数量>`” 来称呼。

例如，接受两个参数的 `sort` 函数叫 `sort/2`，接受三个参数的 `sort` 函数叫
`sort/3`。

大部分运算符（除了 `|>/2` 与 `#/2` 之外）本质上也是通常函数。例如，`-/1`
是取相反数，`-/2` 则是两数相减。

与匿名函数不同，调用通常函数的参数列表前没有 `.`，而是参数列表紧接着函数名，如：

- `sum([1, 2, 3])`

#### 捕获通常函数

如果想把通常函数像匿名函数一样传递给其他函数，需要用到捕获。捕获方式为
`&<函数名>/<参数数量>`。
...
对于通常函数，例如 `at`，`at([1, 2, 3], 2)` 与 `[1, 2, 3] |> at(2)` 是一样的。

对于匿名函数，无论参数列表中是否留有参数，都不能省略 `.`
和参数列表的括号，如：`1 |> (|$x| $x+1).()`。

如果通常函数在使用管道运算符后参数列表为空，可以省略参数列表的括号：`[4, 5, 6] |> product`。
